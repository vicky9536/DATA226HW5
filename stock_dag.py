...
@task
def transform(dict_list, symbol):
  ...
  for i in dict_list:
    ...
    volume = i["5. volume"]
    symbol = symbol
    records.append([timestamp, open, high, low, close, volume, symbol])
...
def load(con, records):
  ...
  try:
    ...
    for r in records:
      timestamp, open, high, low, close, volume, symbol = r
      sql = f"""INSERT INTO {target_table} (date, open, high, low, close, volume, symbol)
...
with DAG(
    ...
    lines = transform(days_records, symbol)
    load(cur, lines)